...
import com.homegenie.paymentservice.exception.PaymentNotFoundException;
...
public class PaymentPlatformServiceImpl implements PaymentPlatformService {
    ...
    @Transactional
    public MiniAppPaymentResponse refundMiniAppPayment(Long paymentId, java.math.BigDecimal amount)
            throws StripeException {
        Payment payment = paymentRepository.findById(paymentId)
                .orElseThrow(() -> new PaymentNotFoundException(paymentId));
        platformConfig.validateMiniApp(payment.getMiniAppId());
        Payment refunded = stripePaymentService.refundPayment(paymentId, amount);
        log.info("Mini-app payment {} refunded", paymentId);
        return MiniAppPaymentResponse.fromPayment(refunded);
    }
    ...
    public MiniAppPaymentResponse createMiniAppPayment(MiniAppPaymentRequest request) throws StripeException {
        ...
        // Optimistic fast-path check; DB unique constraint is the safety net for race conditions
        ...
        try {
            ...
            log.warn("Duplicate order detected via DB constraint for order {}", request.getOrderId());
            try { paymentIntent.cancel(); } catch (Exception ce) {
                log.error("Compensation failed for intent {}: {}", paymentIntent.getId(), ce.getMessage());
            }
            throw new PaymentAlreadyExistsException(request.getOrderId());
        }
    }
